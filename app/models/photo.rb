class Photo
  ...
  attr_writer :contents
    ...
    @id = hash[:_id].to_s if !hash.nil? && !hash[:_id].nil?
    @location = Point.new(hash[:metadata][:location]) if !hash.nil? && !hash[:metadata].nil?
    @place = hash[:metadata][:place] if !hash.nil? && !hash[:metadata].nil?
  end
